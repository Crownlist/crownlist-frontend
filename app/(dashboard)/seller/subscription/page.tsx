...
export default function SellerSubscriptionPage() {
  ...
  useEffect(() => {
    if (paymentStatus === "success") {
      toast.success(
        paymentMessage || "Payment successful! Redirecting to dashboard..."
      );
    ...
    <div className="w-full mx-auto p-4 md:p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              <p className="text-sm opacity-90 mb-1">
                {subscriptionData.data.subscription.status === "active"
                  ? "Current Plan"
                  : "Last Subscribed Plan"}
              </p>
              <h3 className="text-2xl font-bold">
                {subscriptionData.data.subscription.subscriptionPlanName ||
                  "Free Trial"}
              </h3>
            </div>
            ...
            <div>
              ...
              <p className="text-xl font-semibold capitalize">
                {subscriptionData.data.subscription.status}
              </p>
              {subscriptionData.data.subscription.status === "expired" && (
                <p className="text-xs opacity-75 mt-1">
                  Expired on{" "}
                  {formatDate(subscriptionData.data.subscription.expiresAt)}
                </p>
              )}
            </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {plans
            .filter((plan) => plan.name.toLowerCase() !== "free trial")
            .map((plan, idx) => (
              <div key={plan._id} className="border rounded-lg overflow-hidden">
                <div className="p-6">
                  ...
                  <h3 className="text-center text-[#1F058F] font-semibold text-lg mb-2">
                    {plan.name}
                  </h3>
                  ...
                  <Button
                    ...
                    className={
                      idx % 2 === 0
                        ? "w-full border-[#1F058F] text-[#1F058F] hover:bg-[#1F058F] hover:text-white"
                        : "w-full bg-[#1F058F] hover:bg-[#2a0bc0]"
                    }
