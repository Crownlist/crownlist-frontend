...
export default function AdminResourcesPage() {
  ...
  const fetchResources = useCallback(async () => {
    try {
      ...
      const resp = (res as any)?.data ?? {};
      // many responses wrap data in .data.data with items in .data
      const container = resp?.data ?? resp;
      const items = container?.data ?? container?.items ?? container ?? [];
      const totalCount = container?.totalCount ?? resp?.totalCount ?? null;

      const list = Array.isArray(items) ? items : [];

      setResources(list as ResourceItem[]);
      setTotalCount(totalCount);
      setPageHasData(list.length > 0);
    } catch (e: any) {
  ...
  const parseCsvFile = async (file: File) => {
    try {
      ...
      setForm((prev) => ({
        ...prev,
        valueText: Array.from(merged).join("\n"),
      }));
  ...
  // Search filter (no client pagination; server handles pagination)
  ...
  const totalPages = useMemo(() => {
    // If server provides totalCount, use it; otherwise we can't know total pages
    if (typeof totalCount === "number" && totalCount > 0) {
      return Math.max(1, Math.ceil(totalCount / pageSize));
    }
    // If no totalCount, assume there may be more pages
    return Infinity;
  }, [totalCount, pageSize]);

  const currentPage = page;
  // Use filtered directly (already paginated by server)
  ...
  const validate = () => {
    ...
    const lines = form.valueText
      .split(/\r?\n/)
      .map((s) => s.trim())
      .filter(Boolean);
    if (lines.length === 0)
      next.valueText = "Enter at least one value (one per line)";
  ...
  return (
    <div className="p-6">
            ...
            <div className="space-y-4 py-2">
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g., mobile phone brands"
                />
                {errors.name && (
              ...
              <div>
                ...
                <textarea
                  ...
                  value={form.valueText}
                  onChange={(e) => {
                    setForm({ ...form, valueText: e.target.value });
                    if (
                      e.target.value
                        .split(/\r?\n/)
                        .map((s) => s.trim())
                ...
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-500">
                    Upload CSV (single column or comma-separated)
                  </span>
                </div>
              </div>
            </div>
      ...
      ) : resources.length === 0 && page > 1 ? (
        <div className="border rounded-lg p-8 text-center">
          <p className="text-gray-500">No resources on this page</p>
          ...
          <TableBody>
            {pageItems.map((r) => (
              <TableRow key={r._id ?? r.slug ?? r.name}>
                <TableCell
                  className="font-medium text-[#1F058F] cursor-pointer"
                  onClick={() =>
                    router.push(`/admin/resources/${r.slug ?? r.name}`)
                  }
                >
                  {r.name}
                </TableCell>
      ...
      {!loading && resources.length > 0 && (
        <div className="mt-4 flex items-center justify-between gap-3 flex-wrap">
          <div className="text-sm text-gray-600">
            Page {currentPage}
            {totalPages !== Infinity ? ` of ${totalPages}` : ""} •{" "}
            {totalCount ?? filtered.length} result(s)
          </div>
