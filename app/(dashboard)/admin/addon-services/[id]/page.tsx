...
export default function AdminAddonServiceDetailPage() {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof AddOnService, string>>
  >({});
  ...
  const fetchService = async () => {
    ...
    try {
      ...
      const data: AddOnService | undefined =
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const res: any = await apiClientAdmin.put(
        `/addonservices/${id}`,
        payload
      );
      ...
      toast.error(
        e.response?.data?.message ||
          `Failed to update service: ${String(e?.message || e)}`
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="border rounded-lg p-6 space-y-3">
          <div className="flex justify-between">
            ...
            <span className="font-semibold">
              ₦{Number(svc.amount).toLocaleString()}
            </span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-600">Billing Cycle</span>
            <span className="capitalize">{svc.billing_cycle}</span>
          </div>
          <div className="flex justify-between">
        ...
        <Table className="border rounded-lg">
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell className="max-w-[640px] whitespace-pre-wrap">
                {svc.description}
              </TableCell>
            </TableRow>
            <TableRow>
              <TableCell>Amount</TableCell>
              <TableCell>₦{Number(svc.amount).toLocaleString()}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell>Billing Cycle</TableCell>
              <TableCell className="capitalize">{svc.billing_cycle}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell>Billing Type</TableCell>
              <TableCell className="capitalize">{svc.billing_type}</TableCell>
            </TableRow>
            <TableRow>
              <TableCell>Status</TableCell>
              <TableCell className="capitalize">{svc.status}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
      ...
      <Dialog
        open={isEditOpen}
        onOpenChange={(open) => !saving && setIsEditOpen(open)}
      >
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setForm((prev: any) => ({ ...prev!, billing_cycle: value }))
                  }
                  disabled={saving}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setForm((prev: any) => ({ ...prev!, billing_type: value }))
                  }
                  disabled={saving}
                >
                  <SelectTrigger>
