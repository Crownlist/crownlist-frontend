      ...
      <div className="space-y-2 relative">
        ...
        <div className="flex overflow-x-auto no-scrollbar gap-4 py-1">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="min-w-[140px] h-40 rounded-xl bg-gray-200 animate-pulse shrink-0"
            />
          ))}
        </div>
      </div>
      ...
      <div className="flex overflow-x-auto no-scrollbar gap-4 py-1">
        {allCategories.map((category) => {
          ...
          return (
            <div
              key={category._id}
              className={`relative min-w-[140px] h-40 rounded-xl overflow-hidden shrink-0 ${
                !hasSubcategories ? "" : "cursor-pointer"
              }`}
              onClick={() => hasSubcategories && handleCategoryClick(category)}
            >
              <Image
                src={
                  category.imageUrl || "/placeholder.svg?height=160&width=140"
                }
                alt={category.name}
                fill
                className="object-cover"
              />
              <div className="absolute inset-0 bg-linear-to-t from-black/70 via-black/30 to-transparent px-3 py-4 flex flex-col justify-end">
