...
export default function MessagingInterface() {
      ...
      fetchUserChats()
        .then((conversations: any[]) => {
          ...
          setTimeout(() => {
            setIsConversationsLoading(false);
            console.log(
              "🎉 Finished loading conversations. Current state:",
              state.conversations
            );
          }, 500);
        })
  ...
  useEffect(() => {
    if (state.activeConversationId && userProfile?.id) {
      ...
      console.log(
        `🔍 Checking messages for active conversation ${state.activeConversationId}:`,
      ...
      if (existingMessages.length === 0) {
        ...
        import("@/lib/chat-api").then(({ fetchChatMessages }) => {
          fetchChatMessages(state.activeConversationId!)
            .then((response) => {
              console.log(
                "📨 Auto-loaded messages for active conversation:",
              ...
              if (response.messages && response.messages.length > 0) {
                ...
                console.log(
                  "✅ Auto-loaded and merged messages for conversation:",
                  state.activeConversationId!,
                  mergedMessages.length,
                  "messages"
                );
            ...
            .catch((error) => {
              console.error(
                "❌ Failed to auto-load messages for conversation:",
                state.activeConversationId,
                error
              );
  ...
  useEffect(() => {
    ...
    if (chatId && userProfile && !isConversationsLoading) {
      ...
      if (existingConversation) {
        console.log(
          "✅ Found existing conversation, setting as active:",
          existingConversation.id
        );
        actions.setActiveConversation(existingConversation.id);
        // Show conversation on mobile when accessed via direct link
        ...
        console.log(
          "❌ Conversation not found for chat_id:",
          chatId,
    ...
    if (sellerId && sellerName && userProfile && !isConversationsLoading) {
      ...
      if (!existingConversation) {
        ...
        console.log("📡 Creating chat via backend API:", [
          userProfile.id,
          sellerId,
        ]);
          ...
          .then(({ createChat }) => {
            createChat([userProfile.id, sellerId])
              .then((chatResult) => {
                ...
                console.log(
                  "🏗️ Adding conversation with backend ID:",
                  newConversation
                );
                ...
                setTimeout(() => {
                  console.log(
                    "🔄 Setting active conversation for local chat:",
                    newConversation.id
                  );

                  actions.setActiveConversation(newConversation.id);
                  console.log(
                    "✅ Active conversation set to:",
                    newConversation.id
                  );
                  ...
                  console.log(
                    "✅ Auto-created and activated conversation with seller:",
                    sellerName,
                    {
              ...
              .catch((createChatError: any) => {
                ...
                console.log(
                  "🏗️ Adding local conversation due to API failure:",
                  newConversation
                );
                actions.addConversation(newConversation);
        ...
        console.log(
          "✅ Switched to existing conversation with seller:",
          sellerName,
          {
  ...
  const handleReceiveMessage = useCallback(
    (data: ReceiveMessageEvent) => {
      console.log(
        "📨 Received message in chat component:",
        data,
        "userProfile:",
        userProfile
      );
      ...
      if (!chatIdForMessage && state.activeConversation?.id) {
        ...
        console.log(
          "⚠️ No chat_id in message, using active conversation:",
          chatIdForMessage
        );
      }
      ...
      const message: Message = {
        ...data,
        id: `received_${Date.now()}_${Math.random()}_${data.sender}_${
          data.createdAt
        }`,
  ...
  const handleConversationClick = (id: string) => {
    ...
    console.log(
      "🔄 After setActiveConversation, activeConversationId:",
      state.activeConversationId
    );
    setShowMobileConversation(true);
          ...
          fetchChatMessages(id)
            .then((response) => {
              ...
              if (response.messages && response.messages.length > 0) {
                ...
                console.log(
                  "✅ Messages merged successfully for conversation:",
                  id,
                  mergedMessages.length,
                  "total messages"
                );
                ...
                console.log(
                  "ℹ️ No messages to load from API for conversation:",
                  id
                );
              }
            })
  ...
  if (currentMessages.length === 0 && state.activeConversation?.recipientId) {
    ...
    const potentialKey = messageKeys.find((key) =>
      key.includes(state.activeConversation!.recipientId)
    );
    if (potentialKey) {
          ...
          {isConversationsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full py-12 px-4">
              ...
              <h3 className="text-lg font-medium mb-2 text-gray-700">
                ...
                {isConversationsLoading ? "Loading..." : "Empty"})
              </h3>
              <p className="text-gray-500 text-center text-sm">
                Loading: {isConversationsLoading ? "Yes" : "No"} |
                Conversations: {state.conversations.length} | API Call:{" "}
                {userProfile?.id ? "✅" : "❌"}
              </p>
            </div>
          ) : (
          ...
          <>
            <div className="flex justify-between items-center p-4 border-b border-gray-200">
              <div className="flex items-center">
                {showMobileConversation && (
                  <button
                    className="mr-2 lg:hidden text-gray-500"
                ...
                <div className="relative mr-3">
                  <Image
                    src={
                      (state.activeConversation || activeConv)?.avatar ||
                      "/profile.png"
                    }
                    alt={
                      (state.activeConversation || activeConv)?.name ||
                      "Unknown"
                    }
                    width={40}
                    height={40}
