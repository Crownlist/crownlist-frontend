...
import { ChevronRight, ArrowLeft, ArrowRight, Search, ChevronDown, Loader2, SlidersHorizontal } from "lucide-react";
...
export default function SearchCategory({
  ...
  const filteredResults = useMemo(() => {
    ...
    return results;
  }, [category, activeSubcategory, sortOption, searchTerm]);
    ...
    <div className="mx-auto px-4 py-6">
      ...
      <div className="mb-2">
        <div className="flex rounded-lg overflow-hidden border border-gray-300 max-w-80">
          ...
          <Button
            ...
            className="bg-[#1F058F] hover:bg-[#2a0bc0] text-white px-4 py-2"
            onClick={() => {
              console.log("Search button clicked, resetting to page 1");
              setCurrentPage(1);
            }}
          >
            Search
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-[13px] sm:text-lg font-medium">
          {categoryTitle}{subcategoryTitle && ` - ${subcategoryTitle}`}
          <span className="text-gray-500"> ({filteredResults.length} results found)</span>
        </h1>
        <div className="flex items-center gap-4">
          <button
            className="md:hidden flex items-center gap-1 text-sm border rounded px-2 py-1"
            ...
            {sortDropdownOpen && (
              <div className="absolute top-full right-0 mt-1 bg-white shadow-md rounded-md z-10 w-36 py-1">
                {["All", "Lowest price", "Highest price"].map(option => (
                  <button
                    key={option}
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 ${option === sortOption ? "bg-gray-50" : ""}`}
      ...
      <div className="flex flex-col md:flex-row gap-8">
        {/* Commented out: Entire Sidebar Filter Panel */}
        {/*
                    ...
                    {filters.options.map(type => {
                      ...
                      return (
                        <div key={type} className="flex items-center gap-2">
                          <div
                            className={`h-4 w-4 rounded flex items-center justify-center ${isActive ? "bg-green-500 text-white" : "border border-gray-300"}`}
                    ...
                    <div className="max-h-[200px] overflow-y-auto pr-2 space-y-2">
                      {locationOptions.map(location => (
                        <div key={location} className="flex items-center gap-2">
                          <div
                            className={`h-4 w-4 rounded flex items-center justify-center ${selectedLocations.includes(location) ? "bg-green-500 text-white" : "border border-gray-300"}`}
                            onClick={() => toggleLocation(location)}
                          >
                            {selectedLocations.includes(location) && <Check size={12} />}
                          </div>
                          <label className="text-sm cursor-pointer" onClick={() => toggleLocation(location)}>
                            {location}
                          </label>
                        </div>
                      ))}
              ...
              <div className="max-h-[200px] overflow-y-auto pr-2 space-y-2">
                {priceOptions.map(price => (
                  <div key={price} className="flex items-center gap-2">
                    <div
                      className={`h-4 w-4 rounded flex items-center justify-center ${selectedPriceOptions.includes(price) ? "bg-green-500 text-white" : "border border-gray-300"}`}
                      onClick={() => tooglePrice(price)}
                    >
                      {selectedPriceOptions.includes(price) && <Check size={12} />}
                    </div>
                    <label className="text-sm cursor-pointer" onClick={() => tooglePrice(price)}>
                      {price}
                    </label>
                  </div>
                ))}
              ...
              <div className="max-h-[200px] overflow-y-auto pr-2 space-y-2">
                {periodOptions.map(period => (
                  <div key={period} className="flex items-center gap-2">
                    <div
                      className={`h-4 w-4 rounded flex items-center justify-center ${selectedPeriod.includes(period) ? "bg-green-500 text-white" : "border border-gray-300"}`}
                      onClick={() => tooglePeriod(period)}
                    >
                      {selectedPeriod.includes(period) && <Check size={12} />}
                    </div>
                    <label className="text-sm cursor-pointer" onClick={() => tooglePeriod(period)}>
                      {period}
                    </label>
                  </div>
                ))}
              ...
              <div className="flex gap-1">
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-md text-sm ${currentPage === i + 1 ? 'bg-gray-100 font-medium' : 'hover:bg-gray-50'}`}
                  >
                    {i + 1}
                  </button>
                ))}
